...
exports.generateTestInput = void 0;
const sampleWords = [
...
const getRandomChar = (from) => from.charAt(Math.floor(Math.random() * from.length));
const stateCodes = [
...
const generateTestInput = (options = {}) => {
    const { minLength = 5, maxLength = 15, exactLength, allowSpaces = false, charSet = ['alphanumeric'], meaningfulWordsOnly = false, mode = 'default' } = options;
    ...
    if (mode === 'GST') {
        ...
        const pan = getAlpha() + getAlpha() + getAlpha() + getAlpha() + getAlpha() +
    ...
    let charset = '';
    if (charSet.includes('alpha'))
        charset += 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
    if (charSet.includes('numeric'))
        charset += '0123456789';
    if (charSet.includes('alphanumeric'))
        charset += 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    if (charSet.includes('specialChars'))
        charset += specialChars;
    if (allowSpaces)
        charset += ' ';
    if (!charset)
        charset = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'; // fallback to alpha
    let result = '';
